...
| MediaPipe | `@mediapipe/face_mesh` (WASM, browser-only) — **installed** (`v0.4.1633559619`) | Used in `useCameraCheck` for the GO1 camera/face check — both on `/quality-check` and live during recording via `useRecordingMonitor`. WASM + model assets load from the jsDelivr CDN via `locateFile`. Use only via `useCameraCheck`; gated off whenever not recording. |
...
- `useCameraCheck`, `useLightingCheck`, `useNoiseCheck`, `useMicCheck` must each return `{ status: 'CHECKING' | 'PASS' | 'FAIL', message: string }` — no other shape. (The tri-state enum carries the "still measuring" state a boolean can't.)
- `useQualityGate` aggregates the four check hooks for the one-time `/quality-check` gate and returns `{ allPassed: boolean, checks: CheckState }`.
- `useRecordingMonitor({ audioTrack, videoElement, active })` re-runs the GO1 checks **live during recording** and returns `{ checks, failure, clearFailure }` — it raises `failure` when a check stays bad past its grace window (mic 10s, others 5s) so `SessionScreen` can discard the take and prompt a re-record. It reuses the camera/lighting/noise hooks but swaps `useMicCheck` (which latches) for `useMicActivity` (non-latching silence timer). Checks are gated off (null inputs) whenever `active` is false.
- **Stream ownership is split:** `useMediaStream` is the *only* hook that calls `getUserMedia` and the *only* hook that stops tracks (it owns the combined audio+video stream, the `permissionStatus` state machine, and `errorKind` classification). `useMediaRecorder` and the GO1 check hooks (incl. `useMicActivity`) are *consumers* — they receive the stream/track/element as an argument and must never call `getUserMedia` or stop a track, so the live preview keeps running after a recording stops. See ARCHITECTURE.md → "Media Stream & Permission Flow".
- **Skip the gate when already permitted:** on mount `useMediaStream` probes the Permissions API for `camera` + `microphone`. If **both** are already `granted`, it acquires the stream silently via the `checking` state (no learner tap), so `/quality-check` shows the live camera and `/session` jumps straight to recording. The `permissionStatus` machine has five states — `checking` → `pending` → `requesting` → `granted` / `denied`. The native prompt still only fires from `requestStream()` after a tap (the explanation-before-prompt invariant): `requesting` is reserved for that post-tap path, so the auto-acquire path uses `checking` and never flashes the explainer. Any uncertain probe (Firefox `TypeError`, Safari no-API, state `prompt`) falls back to `pending`. `PrePermissionScreen` is the one allowed exception that calls `getUserMedia` outside the hook — purely to trigger the grant — and **must stop its probe stream's tracks immediately** so `/quality-check` can re-acquire without an "in use" error.
...
- **Implemented:** `useMediaStream`, `useMediaRecorder`, `useAnalyzeSubmit` (FastAPI POST state machine — AbortController + 120s timeout + abort-on-unmount), all four GO1 check hooks (each returns `{ status: 'CHECKING' | 'PASS' | 'FAIL', message }`: `useCameraCheck` — face-mesh check that dynamically imports `@mediapipe/face_mesh`, auto-passes on a 10s WASM-load timeout or load failure, with forgiving centering/proximity thresholds so the face need not fill the oval guide; `useMicCheck` — Web Audio amplitude that latches to PASS once it hears one clear in-range sample and then stays passed, so the learner can go quiet again; `useNoiseCheck` — Web Audio ambient-noise floor; `useLightingCheck` — Canvas luminance), `useMicActivity` (non-latching recording-time mic silence timer), `useQualityGate` (aggregates the four checks for `/quality-check` → `{ allPassed, checks }`), `useRecordingMonitor` (live GO1 watchdog during recording → `{ checks, failure, clearFailure }`), `useSessionStorage` (wraps `saveSessionRecord` with a StrictMode once-guard → `{ saveStatus }`), `useAuth` (+ `AuthContext`/`AuthGuard`/`useCurrentUser`), `useSessionHistory`, `usePassage` (fetches the passage bank from Supabase + random pick → `{ passage, loading, error, retry }`), `lib/supabase.js` (client + `saveSessionRecord` + `fetchPassages` — the `passages` table is the single source of truth; there is no local passage bank), `lib/api.js` (real `submitRecording` fetch), `lib/philIri.js`, `lib/dashboardStats.js`, `utils/platform.js`, all six pages (`LoginPage`, `PrePermissionScreen`, `QualityCheckScreen`, `SessionScreen`, `SessionResultsPage`, `DashboardPage`), and the `quality/` (`CameraPreview`, `PermissionExplainer`, `PermissionDenied`), `session/` (`RecordingChecks` — the live GO1 indicator strip), `results/`, `dashboard/`, `ui/`, and `layout/` component folders.
- **Stubs / fake data:** none in `lib/api.js` anymore — `submitRecording()` performs the real FastAPI `POST /analyze`. (The `/results` page keeps a dev-only fallback that calls `submitRecording` directly when reached without route state.)
- **Referenced in docs but not yet created:** the `src/components/session/` folder now exists but holds only `RecordingChecks.jsx`; the other components older docs name (`RecordingGate`/`ProcessingScreen`/`PassageDisplay`/`CheckIndicator`) are still not created — the processing UI lives inline in `pages/SessionScreen.jsx`, and the GO1 `CheckIndicator` lives inline in `pages/QualityCheckScreen.jsx`. ARCHITECTURE.md's folder tree was refreshed to match the real `src/` (2026-05); even so, trust the real tree over any document. (`utils/readingLevel.js` from older docs is actually `lib/philIri.js`.)
...
- Do not add loading spinners or skeleton screens that block the GO1 checks or the passage display — the GO1 four-check panel (on `/quality-check`) and the live camera preview must be visible and live as soon as the camera stream is available.